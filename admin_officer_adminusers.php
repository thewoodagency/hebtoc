...
if (isset($_SESSION['admin']) && $_SESSION['admin'] == TOCEMAIL) {
	...
	$qString = "select * from toc_admin order by adminid";
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	...
	while ($row = $r->fetch_assoc()) {
		...
		$tbrows .= '<td>' . '<a target="_blank" href="_editAdminuser.php?remail=' . $row["idtoc_admin"] . '">' .
			$row["adminid"] . '</td><td>' .
			//$row["adminpwd"] . '</td><td>' .
			$row["department"] . '</td><td>' .
			$row["lastlogin"] . '</td><td align="center">' .
			'<a href="javascript:remove(' . $row["idtoc_admin"] . ',\'' . $row["adminid"] . '\')"><img src="images/remove.gif" border="0"></a></td></tr>';
		$rowcolor++;
	}
} else {
...
<script>
	function remove(rid, name) {
		...
		if (r == true) {
			window.location = "_removeRecord_Adminuser.php?rid=" + rid;
		} else {
...
<html>
<!-- InstanceBegin template="/Templates/adminPage.dwt.php" codeOutsideHTMLIsLocked="false" -->
...
<body>
	...
	<div id="tocform">
		...
		<div id="content">
			...
			<p><a style="background-color:#FF0" href="_editAdminuser.php?new=true">Create a new admin user</a></p>
		</div>
		<!-- InstanceEndEditable -->
	</div>
	<div id="footer">
