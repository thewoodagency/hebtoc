...
on toc_regid=idtoc_register where toc_firstname <> \'\' and toc_topgolf2 = 1 order by company, toc_lastname'; //topgolf2 === spa
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	while ($row = $r->fetch_assoc())
	{
		...
		$tbrows .= '<td>' . $row["toclevel"] . '</td><td>' .
                    ...
                    $row["toc_title"] . '</td>' . '<input type="hidden" name="hid'.$rowcolor.'" value="'.$row["hid"].'">' .
                    '<input type="hidden" name="gloc'.$rowcolor.'" id="gloc'.$rowcolor.'" value="' . $row["toc_topgolf_loc"] . '"><td>' .
					$row["toc_bdm"] . '</td></tr>';
					$rowcolor++;
	}
} else {
...
<div id="content">
<p><a href="admin_golf4_excel.php">Excel</a></p>
    <form class="jotform-form" action="admin_golf_proc.php" method="post" name="form_32946254898168" id="32946254898168" accept-charset="utf-8">
        ...
        <input type="hidden" name="golf" value="4" /><!-- 4 === spa -->
