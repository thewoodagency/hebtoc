...
if(isset($_SESSION['email']) && isset($_POST['formID']))
{
	...
	for ($i=0; $i<$noofPeople; $i++)
	{
		...
		//{
			...
			$hid=$_POST['hid'.$i];
		//print_r($room);
			$qString = sprintf('replace into toc_tennis (hid, regID, regEmail, hfirst, hlast, hemail, hrating) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s")',
	...
	$dbc->close();
	//send_notice("hotel");
	header("Location: reg_tennis.php");
} else {
