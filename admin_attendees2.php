...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toc_attendees.hid as hid, toc_hall.regID as rid, toc_register.company as co, toc_hall.htitle as title, toc_hall.hfirst as first, toc_hall.hlast as last, welcome, toc_attendees.summit ' .
'FROM toc_hall left join toc_attendees ' .
'on toc_attendees.regID=toc_hall.regid and toc_attendees.hfirst = toc_hall.hfirst and toc_attendees.hlast = toc_hall.hlast ' .
'inner join toc_register on toc_hall.regID=idtoc_register ' .
'inner join toc_level on toclevel = tlName where tlAmount * 1 <= 100000 and toc_hall.hlast <> \'\'' ;
	$r = $dbc->query($qString);
	$qStringInsert = '';
	$i=0; $j=0;
	...
	{	 
		...
		if(is_null($row['summit'])) { $summit = 'No'; } else {$summit = $row['summit']; if ($summit=='Yes') $i++;}
		echo "<span style='color:#FFF;font-weight:bold;'>" . $row["co"] . "-summit - " . $row["summit"] . "|" . $summit . "</span><br>";
		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit, general) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
