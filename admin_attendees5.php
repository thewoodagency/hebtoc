...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toc_attendees.hid as hid, toc_charity.regID as rid, toc_register.company as co, toc_charity.htitle as title, toc_charity.hfirst as first, toc_charity.hlast as last, welcome, toc_attendees.summit, general, halloffame, private ' .
'FROM toc_charity left join toc_attendees ' .
'on toc_attendees.regID=toc_charity.regid and toc_attendees.hfirst = toc_charity.hfirst and toc_attendees.hlast = toc_charity.hlast ' .
'inner join toc_register on toc_charity.regID=idtoc_register ' .
'inner join toc_level on toclevel = tlName where toc_charity.hlast <> \'\'' ;
	$r = $dbc->query($qString);
	$qStringInsert = '';
	$i=0; $j=0;
	...
	{	 
		...
		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit, general, halloffame, private, charity) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
