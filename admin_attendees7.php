...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toc_attendees.hid as hid, toc_golf.regID as rid, toc_register.company as co, toc_golf.htitle as title, toc_golf.hfirst as first, toc_golf.hlast as last, welcome, toc_attendees.summit, general, halloffame, private, charity, golf1 ' .
...
'inner join toc_level on toclevel = tlName where toc_level.tlamount < 125000 and toc_golf.hlast <> \'\'' ;
	...
	{	 
		...
		if(is_null($row['golf1'])) { $golf1 = 'No'; } else {$golf1 = $row['golf1'];}

		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit, general, halloffame, private, charity, golf1, golf2) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
