...
if(isset($_SESSION['email']) && isset($_POST['formID']))
{
	...
	for ($i=0; $i<$noofPeople; $i++)
	{
		...
		//{
			...
			$size=$_POST['size'.$i];
			$hid=$_POST['hid'.$i];
			$waiver=$_POST['waiver'.$i];
			$signed=$_POST['signed'.$i];
	
			$qString = sprintf('replace into toc_charity (hid, regID, regEmail, hfirst, hlast, htitle, hsize, waiver, signed) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
	...
	$dbc->close();
	//send_notice("hotel");
	header("Location: reg_charity.php");
} else {
