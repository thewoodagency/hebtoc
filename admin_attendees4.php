...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toc_attendees.hid as hid, toc_pdinner.regID as rid, toc_register.company as co, toc_pdinner.htitle as title, toc_pdinner.hfirst as first, toc_pdinner.hlast as last, welcome, toc_attendees.summit, general, halloffame ' .
'FROM toc_pdinner left join toc_attendees ' .
'on toc_attendees.regID=toc_pdinner.regid and toc_attendees.hfirst = toc_pdinner.hfirst and toc_attendees.hlast = toc_pdinner.hlast ' .
'inner join toc_register on toc_pdinner.regID=idtoc_register ' .
'inner join toc_level on toclevel = tlName where toc_pdinner.hlast <> \'\'' ;
	$r = $dbc->query($qString);
	$qStringInsert = '';
	$i=0; $j=0;
	...
	{	 
		...
		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit, general, halloffame, private) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
