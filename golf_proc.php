...
if(isset($_SESSION['email']) && isset($_POST['formID']))
{
	...
	{
		//if (isset($_POST['hseq'.$i]) && $_POST['hseq'.$i]<>'')
		//{
			$fname=$_POST['first'.$i];
			$lname=$_POST['last'.$i];
			$title=$_POST['title'.$i];
			$bdm=$_POST['bdm'.$i];
			$flight=$_POST['flight'.$i];
			$hid=$_POST['hid'.$i];
			$seq=$_POST['hseq'.$i];
		//print_r($room);
			$qString = sprintf('replace into toc_golf (hid, regID, regEmail, hfirst, hlast, htitle, hbdm, hflight, hseq) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
			...
			$dbc->real_escape_string($flight),
			$dbc->real_escape_string($seq));
			$dbc->query($qString);
		//}
	} //end for
	$dbc->close();
	header("Location: reg_golf.php");
	//send_notice("hotel");
} else {
