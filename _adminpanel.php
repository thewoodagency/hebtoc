...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'select * from toc_register order by idtoc_register, toclevel';
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	while ($row = $r->fetch_assoc())
	{
		...
		$tbrows .= '<td>' . $row["toclevel"] . '</td><td>' .
					$row["company"] . '</td><td><a href="adminoverride.php?source=reg_info&email=' . $row["email"] . '" target="_blank">' .
					$row["email"] . '</a></td><td>' .
					$row["password"] . '</td><td>' .
					$row["registerd_date"] . '</td><td><a href="adminoverride.php?source=_invoice&email=' . $row["email"] . '" target="_blank">invoice</a></td></tr>';
					$rowcolor++;
	}
} else {
	include 'logout.php';
...
<div id="content">
<table><tr><td>Sponsorship</td><td>Company</td><td>Email</td><td>Password</td><td>Registered Date</td></tr>
<? echo $tbrows; ?>
</table>
</div>
<!-- InstanceEndEditable -->
