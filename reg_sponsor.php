...
if(isset($_SESSION['email']))
{
	...
	//$qString = sprintf('select * from toc_summit where regEmail="%s" order by hid', $dbc->real_escape_string($regEmail));
    $qString = sprintf('select * from toc_summit where hfirst <> \'\' and regID="%s" and regEmail="%s" order by hid',
...
<form class="jotform-form" action="sponsor_proc.php" method="post" name="form_32946254898168" id="32946254898168" accept-charset="utf-8">
  <input type="hidden" name="formID" value="32946254898168" />
  <input type="hidden" name="noofPeople" value="<? echo $tocInfo['summit'] ?>">
  <div class="form-all">
  <? if ($tocInfo['summit'] == 0) { ?>
      ...
      <li class="form-line" id="id_4">
        <div id="cid_4" class="form-input-wide">
          <table summary="" cellpadding="4" cellspacing="0" class="form-matrix-table">
            <!-- header -->
            <tr><td colspan="7"><h3><? echo $tocInfo['summith'] ?></h3>
            <strong><? echo $tocInfo['summitm'] ?></strong></td></tr>
            <tr>
