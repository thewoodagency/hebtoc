...
if(isset($_SESSION['email']))
{
	...
	$playtopgolf = true; //true - sponsor can't update golf info || false - sponsor can update golf info
	
	$requestPlayer = "BDM Request";
	if ($toclevel == "Humanitarian") {
		$requestPlayer = "H-E-B Officer/Director/BDM Request";
		$playtopgolf = true;
	} else if ($toclevel == "Presidential" || $toclevel == "Chairman") {
		$requestPlayer = "H-E-B Director/BDM Request";
		$playtopgolf = true;
	}
	
	//$qString = sprintf('select * from toc_golf where regEmail="%s" order by hseq, hid, hflight', $dbc->real_escape_string($regEmail));
    $qString = sprintf('select * from toc_golf where hfirst <> \'\' and regID="%s" and regEmail="%s" order by hseq, hid, hflight',
...
<form class="jotform-form" action="golf_proc.php" method="post" name="form_32946254898168" id="32946254898168" accept-charset="utf-8">
  <input type="hidden" name="formID" value="32946254898168" />
  <input type="hidden" name="noofPeople" value="<? echo $tocInfo['golf'] ?>">
  <div class="form-all">
  <? if ($tocInfo['golf'] == 0) { ?>
      ...
      <li class="form-line" id="id_4">
        <div id="cid_4" class="form-input-wide">
          <table summary="" cellpadding="4" cellspacing="0" class="form-matrix-table">
            <!-- header -->
            <tr><td colspan="7"><h3><? echo $tocInfo['golfh'] ?></h3><strong><? echo $tocInfo['golfm'] ?></strong></td></tr>
            <tr>
			...
			for ($i=0; $i<$tocInfo['golf']; $i++)
			{
				...
				$bdm='';
				$flight='value="AM"';
				if ($i>=$tocInfo['golf']/2) $flight='value="AM"'; //'value="PM"'
				
				if ($toclevel == 'Torchbearer' || $toclevel == 'Platinum' || $toclevel == 'Gold') $flight='value="AM"';
        ...
        <div id="cid_3" class="form-input"><span class="form-sub-label-container">'.($i+1).'&nbsp;<input type="hidden" name="hid'.$i.'" value="'.$hid.'"><input type="hidden" name="hseq'.$i.'" value="'.($i+1).'"><input class="form-textbox" type="text" size="15" name="first'.$i.'" id="first'.$i.'" ' . $firstname . ' />
            <label class="form-sub-label" for="first_3" id="sublabel_first"> First Name </label></span><span class="form-sub-label-container"><input class="form-textbox" type="text" size="15" name="last'.$i.'" id="last'.$i.'" ' . $lastname . ' />
            <label class="form-sub-label" for="last_3" id="sublabel_last"> Last Name </label></span>
        </div>
              </th>
              <td align="center" class="form-matrix-values">
                <input type="text" class="form-textbox" id="title'.$i.'" name="title'.$i.'" size="15" ' . $title . '/>
              </td>
              <td align="center" class="form-matrix-values">
          		<input type="text" class="form-textbox" id="bdm'.$i.'" name="bdm'.$i.'" size="15" ' . $bdm . '/>
              </td>
			  <td align="center" class="form-matrix-values">
          		<input type="text" readonly class="form-textbox" id="flight'.$i.'" name="flight'.$i.'" size="1" ' . $flight . '/>
              </td>
            </tr>';
        ...
        <div id="cid_2" class="form-input-wide">
          <div style="text-align:left" class="form-buttons-wrapper">
			  <? if($playtopgolf) {
					...
					include 'reg_submit_kathyOnly.php';
				}
