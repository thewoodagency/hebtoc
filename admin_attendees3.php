...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toc_attendees.hid as hid, toc_hall.regID as rid, toc_register.company as co, toc_hall.htitle as title, toc_hall.hfirst as first, toc_hall.hlast as last, welcome, toc_attendees.summit, general ' .
...
'inner join toc_level on toclevel = tlName where tlAmount*1 > 100000 and toc_hall.hlast <> \'\'' ;
	...
	{	 
		...
		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit, general, halloffame) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
