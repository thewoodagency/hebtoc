...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT toclevel, tlAmount, company, hfirst, hlast, htitle, hsize, waiver, signed FROM toc_charity inner join toc_register on regID=idtoc_register
inner join toc_level on toclevel = tlName where hlast <> \'\' order by company, hlast';
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	while ($row = $r->fetch_assoc())
	{
		...
		$waiver = '';
		if($row["waiver"] == 1) $waiver = '<img src="images/check4.gif">';
        $tbrows .= '<td>' . trim($row["toclevel"]) . '</td><td>' .
					$row["company"] . '</td><td>' .
					$row["hlast"] . ', ' .$row["hfirst"] . '</td><td>' .
					$row["htitle"] . '</td><td>' . 
					$row["hsize"] . '</td><td>' .
                    $row["signed"] . ' ' . $waiver . '</td></tr>';
					$rowcolor++;
	}
} else {
...
<thead><tr><th>Sponsorship level</th><th>Company Name</th><th>Attendees</th><th>Title</th><th>Shirt Size</th><th>Waiver</th></tr></thead>
<? echo $tbrows; ?>
</table>
</div>
<!-- InstanceEndEditable -->
