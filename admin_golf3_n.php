...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == TOCEMAIL)
{
	...
	$qString = 'SELECT hid, toclevel, company, toc_broker_company, toc_firstname, toc_lastname, toc_title, toc_bdm, toc_golf_loc FROM toc_events inner join toc_register 
on toc_regid=idtoc_register where toc_firstname <> \'\' and toc_golf = 1 order by company, toc_lastname'; //regular golf
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	while ($row = $r->fetch_assoc())
	{
		...
		$tbrows .= '<td>' . $row["toclevel"] . '</td><td>' .
                    ...
                    '<input type="text" name="gloc'.$rowcolor.'" id="gloc'.$rowcolor.'" value="' . $row["toc_golf_loc"] . '"></td><td>' .
					$row["toc_bdm"] . '</td></tr>';
					$rowcolor++;
	}
} else {
...
<div id="content">
<p><a href="admin_golf3_excel.php">Excel</a></p>
    <form class="jotform-form" action="admin_regular_golf_proc.php" method="post" name="form_32946254898168" id="32946254898168" accept-charset="utf-8">
        <input type="hidden" name="formID" value="32946254898168" />
        <input type="hidden" name="numRows" value="<?php echo $rowcolor ?>" />
        <input type="hidden" name="golf" value="3" /><!-- 3 === regular golf -->
