...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	$qString = 'SELECT hid, toclevel, company, toc_broker_company, toc_firstname, toc_lastname, toc_title, toc_bdm, toc_topgolf_loc FROM toc_events inner join toc_register 
on toc_regid=idtoc_register where toc_firstname <> \'\' and toc_topgolf = 1 and toclevel not in ("Humanitarian", "Chairman", "Presidential") order by company, toc_lastname';
	$r = $dbc->query($qString);
	$tbrows = '';
	$rowcolor = 0;
	while ($row = $r->fetch_assoc())
	{
		...
		$tbrows .= '<td>' . $row["toclevel"] . '</td><td>' .
                    ...
                    $row["toc_title"] . '</td><td>' . '<input type="hidden" name="hid'.$rowcolor.'" value="'.$row["hid"].'">' .
                    '<input type="text" name="gloc'.$rowcolor.'" id="gloc'.$rowcolor.'" value="' . $row["toc_topgolf_loc"] . '"></td><td>' .
					$row["toc_bdm"] . '</td></tr>';
					$rowcolor++;
	}
} else {
...
<div id="content">
<p><a href="admin_golf2_excel.php">Excel</a></p>
    <form class="jotform-form" action="admin_golf_proc.php" method="post" name="form_32946254898168" id="32946254898168" accept-charset="utf-8">
        <input type="hidden" name="formID" value="32946254898168" />
        <input type="hidden" name="numRows" value="<?php echo $rowcolor ?>" />
        <input type="hidden" name="golf" value="2" />
