...
if(isset($_SESSION['admin']) && $_SESSION['admin'] == 'kashwin50@hotmail.com')
{
	...
	//Summit Attendees
	$qString = 'SELECT toc_attendees.hid as hid, toc_summit.regID as rid, toc_register.company as co, toc_summit.htitle as title, toc_summit.hfirst as first, toc_summit.hlast as last, welcome ' .
				'FROM toc_summit left join toc_attendees ' .  
                 'on toc_attendees.regID=toc_summit.regid and toc_attendees.hfirst = toc_summit.hfirst and toc_attendees.hlast = toc_summit.hlast ' . 
				  'inner join toc_register on toc_summit.regID=idtoc_register where toc_summit.hlast <> \'\'';
	$r = $dbc->query($qString);
	$qStringInsert = '';
	...
	{	
		...
		$qStringInsert .= sprintf('replace into toc_attendees (hid, regID, hfirst, hlast, title, company, welcome, summit) values ("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")',
